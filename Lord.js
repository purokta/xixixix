...
module.exports = Zeeone = async (Zeeone, Ofc) => {
	try {
		...
		const alphaNumber = [`6282139089053@s.whatsapp.net`, `994405918990@s.whatsapp.net` ]
...
var waktoo = date.getHours();
		...
		switch(bulan1) {
			...
			var tampilTanggal = "" + hari + ", " + tanggal + ", " + bulan1 + ", " + tahun;
				var tampilWaktu = "" + jam + " O'clock , " + menit + " Minute , " + detik + " Second";
            

			const time2 = moment().tz('Asia/Jakarta').format('HH:mm:ss')
			if(time2 < "23:59:00"){
				var ucapannya = `sugeng dalu🌑${pushname}`
				}
       	 if(time2 < "19:00:00"){
     		   var ucapannya = `sugeng dalu🌑 ${pushname}`
				}
     	   if(time2 < "18:00:00"){
     		   var ucapannya = `sugeng sonten🌅 ${pushname}`
				}
      	  if(time2 < "15:00:00"){
     		   var ucapannya = `sugeng siang🏖️ ${pushname}`
				}
    	    if(time2 < "11:00:00"){
       		 var ucapannya = `sugeng enjing🌄 ${pushname}`
				}
    	    if(time2 < "05:00:00"){
  		      var ucapannya = `sugeng dalu🌑 ${pushname}`
				}
				mess = {
					wait: 'wait data is being processed',
